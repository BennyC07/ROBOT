...
try:
    while True:
        ...
        # --- Side-wall poll (cached so line-loss routing is instant) ------
        if state == "TRACK" and time.ticks_diff(now, side_poll_ms) >= SIDE_POLL_INTERVAL_MS:
            side_poll_ms = now
            side_left_dist  = us_left.read()
            side_right_dist = us_right.read()

        # --- TRACK --------------------------------------------------------
        if state == "TRACK":
            if position is not None:
                smooth_pos = (smooth_pos * SMOOTH_WEIGHT + position) // (SMOOTH_WEIGHT + 1)
                ...
                # Line just went missing. Brief coast to ride out flickers,
                # then route on cached side-wall readings - no pause.
                search_counter += 1
